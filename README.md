...
- **좌석 선택 화면**
  - 공연장의 좌석 배치를 한눈에 볼 수 있고, 원하는 좌석을 직접 클릭해서 선택/해제할 수 있습니다.
  - 선택된 좌석은 하단에 요약으로 표시되어, 어떤 좌석을 골랐는지 바로 확인할 수 있습니다.
  - **웹소켓을 사용해 다른 사용자가 좌석을 예약하면 해당 좌석이 실시간으로 UI에 반영되며, 즉시 선택 불가 상태로 변경됩니다.**

- **결제 정보 확인 페이지**
  - 선택한 공연, 날짜, 좌석, 인원, 총 결제 금액을 한 화면에서 정리해서 보여줍니다.
  - 결제 전에 내가 선택한 정보가 맞는지 다시 확인하고 수정할 수 있는 단계입니다.

- **임시 예약(좌석 홀딩)**
  - 좌석을 선택한 뒤 **‘결제하기’ 버튼을 누르면**, 선택한 좌석이 **최대 30분 동안 임시 예약 상태**로 잡힙니다.
  - 이 시간 안에 결제가 완료되면 임시 예약이 **최종 예약**으로 전환되고,
    결제가 완료되지 않으면 약 **30~35분 사이에 자동으로 예약이 취소**되어 다른 사용자가 다시 선택할 수 있습니다.

- **결제 모달 연동**
  - ‘토스로 결제하기’ 버튼을 누르면 실제 결제 모달이 열리고, 여러 결제 수단 중 원하는 방식을 선택해 결제를 진행할 수 있습니다.
  - 결제가 완료되면 예약이 확정되고, 이후 마이페이지 예매 내역에서 결과를 확인할 수 있습니다.
...
- **공연 결제 완료 페이지 (SSR)**
  - 토스 결제 완료 후, 서버에서 실제 결제/예약 정보를 다시 검증한 뒤 예매 내역을 렌더링합니다.
  - 공연명, 좌석, 인원, 결제 금액, 예매 번호, QR 코드 등을 한 화면에서 확인할 수 있어 결제 이후 동선이 명확합니다.
  - 사용자는 이 페이지에서 바로 마이페이지 예매 내역으로 이동하거나, QR 코드를 이용해 현장에서 바로 입장할 수 있습니다.

- **상점 결제 완료 페이지 (SSR)**
  - 상점에서 아이템을 구매하면, 결제 완료 페이지에서 구매한 상품 목록과 사용된 포인트/금액을 요약해 보여줍니다.
  - 서버에서 결제 결과와 사용자 포인트 정보를 다시 조회해 렌더링하기 때문에, 새로고침해도 항상 일관된 데이터를 확인할 수 있습니다.
  - 이후 마이페이지의 구매 이력과도 자연스럽게 이어지도록 동선을 구성했습니다.
