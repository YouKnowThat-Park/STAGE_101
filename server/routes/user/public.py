...
from .helpers import _unlink_kakao, create_default_data_for_new_user
...
def delete_user(data: DeleteUserRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if is_social:
        if data.agreement_text != "동의합니다":
            raise HTTPException(status_code=400, detail="탈퇴 동의 문구를 정확히 입력해 주세요.")
    else:
...
def get_public_user_profile(user_id: str, db: Session = Depends(get_db)):
